...
'use strict';
//объявление моих дополнительных переменных


//конец
var DEFAULT_URL = 'getting.pdf';
...
window.addEventListener('pagechange', function pagechange(evt) {
  ...
  if (evt.previousPageNumber !== page) {
      ...
      //мой комментарий
        //document.getElementById("result").innerHTML = MY_SPECIAL_BOOK_ID;
        
        var xhr = new XMLHttpRequest();
        //xhr.open('GET', '../webresources/gen/getab?name=' + page, true);
        xhr.open('GET', '../webresources/generic/lastpage?page=' + page + '&bookId=' + getCookie('name'), true);
        xhr.send(null);
        //пока проблема такая: кладётся номер страницы, с которой ушли, а не той, на которую пришли. Возмжожно, тут даже косяк с тем, что сохраняется изначальная страница. Что является большой ошибкой.
        //проблемы нет. всё сохраняется корректно
        ///////
